...
if __name__ == '__main__':
    ...
    print('total rewards::',array_total_rewards)
    print('100 iteration average rewards::',array_avg_rewards)
    torch.save(agent.target_q_model.state_dict(), 'models/agent_target_q_model_per_dqn.pth')
    torch.save(agent.q_model.state_dict(), 'models/agent_q_model_per_dqn.pth')
    print('******************* Total Images Processed *************************', agent.samples_processed)
    plt.xlabel('episode')
    plt.ylabel('total reward')
    plt.plot(array_avg_rewards)
    plt.savefig('rewards_avg_100_per_dqn.png')
    
    plt.xlabel('episode')
    plt.ylabel('total reward')
    plt.plot(array_total_rewards)
    plt.savefig('rewards_per_dqn.png')
