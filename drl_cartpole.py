...
class DQNAgent:
  ...
  def get_batch(self):
    batch=self.extract_batch_from_mem()
    if batch==None:
      return
    ...
    return states,actions,next_states,rewards,valid_ns_indices

  def train_q_model(self):
    if self.get_batch() is None:
      return
    states,actions,next_states,rewards,valid_ns_indices=self.get_batch()
...
if __name__ == '__main__':
    ...
    print('total rewards::',array_total_rewards)
    print('100 iteration average rewards::',array_avg_rewards)
    torch.save(agent.target_q_model.state_dict(), 'models/agent_target_q_model_double_dqn.pth')
    torch.save(agent.q_model.state_dict(), 'models/agent_q_model_double_dqn.pth')

    plt.xlabel('episode')
    plt.ylabel('total reward')
    plt.plot(array_avg_rewards)
    plt.savefig('rewards_avg_100_double_dqn.png')
    
    plt.xlabel('episode')
    plt.ylabel('total reward')
    plt.plot(array_total_rewards)
    plt.savefig('rewards_double_dqn.png')